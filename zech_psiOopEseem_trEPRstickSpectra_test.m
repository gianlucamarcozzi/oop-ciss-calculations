...
xxSim1 = linspace(344.25, 347.25, 301);  % mT
...
dgax = min(glw1, glw2) / 10;
smallLw_ = min(lw1, lw2)*1e-1*1e-3;  % GHz

LtrSignalPowder = zeros(numel(xxSim1'), 1);
...
%     waitbar(ii/nSolidAngle, wb, append(string(ii), ' out of ', string(nSolidAngle)));
    ...
    gaussianFactors1 = gaussianFactors;
    [gaussianFactors, idxs1, idxs2] = restrictgaussianfactors(...
        gaussianFactors, gInh1, gInh2, 2);
...
%     gaussianFactors = reshape(gaussianFactors, [1, nPair]);
    ...
    w0_ = g2wFunc(xxSim1', gPlus);
    deltaw_ = g2wFunc(xxSim1', gMinus);
    sintheta_ = sqrt( nVers(1, ii)^2 + nVers(2, ii)^2 );

%     trSignal__ = zeros(numel(xxSim1), nPair);
    trSignal__ = zeros(numel(xxSim1), nPair);
    quantNumNuc = -nNuc/2:1:nNuc/2;
    signJmd = [-1, 1, -1, 1];
    signOmega = [-1, -1, 1, 1];
...
%             toc
            ...
            %         % intensityAtReson_*pascalFactor(ihfi);
            % end
%             smallLw = 10*(mt2mhz(xxSim1(2) - xxSim1(1)))*1e-3
            trSignal__ = ...
                gaussianbs(wReson*1e-3, mwFreq, smallLw_, "lwpp");
%             trSignal__ = datasmooth(trSignal__, 1);
            % trSignal = trSignal + intensityReson.*trSignal_*pascalFactor(ihfi);
            trSignal__ = intensityReson.*trSignal__*pascalFactor(ihfi).*gaussianFactors;
            % trSignal__ = trSignal__.*gaussianFactors;
            LtrSignalPowder = LtrSignalPowder + sum(trSignal__, 2)*sintheta_;
        end
    end
...
% delete(wb)
% clear("wb")
% nLtrSignalPowder = LtrSignalPowder;
nLtrSignalPowder = LtrSignalPowder/max(abs(LtrSignalPowder));
% clf
% plot(xxSim1, ntrSignal, 'o')
hold on
plot(xxSim1, nLtrSignalPowder, 'o-')
hold on
...
Par.nTheta = nTheta;
Par.nPhi = nPhi;
Par.nSolidAngle = nSolidAngle;
Par.dgax = dgax;
Par.lw1 = lwArray(ilw1);
Par.lw2 = lwArray(ilw2);
Par.elapsTime = elapsTime;
iSimul = ilw2 + nlw*(ilw1 - 1);
iSimul = char(string(iSimul));
while numel(iSimul) < 3
    iSimul = ['0' iSimul];
end
filename_ = append('data/generated/ZePSI-simul-002', iSimul, '.mat');
% save(filename_, 'LtrSignalPowder', 'Par');

% disp(append(newline, ...
%     'nTheta x nPhi = ', string(nTheta), ' x ', string(nPhi), ' = ', ...
%     string(nSolidAngle), newline, ...
%     'dgax = ', string(dgax), newline, ...
%     'Lwpp1,2 = ', string(mhz2mt(lw1)), ' mT, ', string(mhz2mt(lw2)), ' mT', newline, ...
...
nexttile
imagesc(reshape(gaussianFactors1, [ngax, ngax]))
colormap(viridis())
colorbar
nexttile
imagesc(reshape(gaussianFactors, [numel(idxs1), numel(idxs2)]))
colormap(viridis())
colorbar
nexttile
plot(gax, gInh1, 'o', gax, gInh2, 'o')
nexttile
%}
...
function [newMatr, idxs1, idxs2] = restrictgaussianfactors(matr, xx1, xx2, xxFactor)
    [~, idxMax1] = max(xx1);
    [~, idxMax2] = max(xx2);
